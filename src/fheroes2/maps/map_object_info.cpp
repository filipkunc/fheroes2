...
namespace
{
    ...
    // This map is used for resolving an original map object's main ICN sprite back to the corresponding editor object.
    ...
    void populateObjectData()
    {
        ...
        for ( size_t groupIndex = 0; groupIndex < objectData.size(); ++groupIndex ) {
            const auto & objects = objectData[groupIndex];
            for ( size_t objectIndex = 0; objectIndex < objects.size(); ++objectIndex ) {
                ...
                const Maps::ObjectPartInfo & mainPart = objectInfo.groundLevelParts.front();
                mainObjectByIcn.try_emplace( std::make_pair( mainPart.icnType, mainPart.icnIndex ),
                                             std::make_pair( static_cast<Maps::ObjectGroup>( groupIndex ), static_cast<uint32_t>( objectIndex ) ) );
            }
        }
