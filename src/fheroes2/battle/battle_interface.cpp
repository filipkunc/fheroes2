...
namespace
{
    ...
    // Convert a palette color index to 8-bit RGB.
    void paletteColorToRGB( const uint8_t colorId, uint8_t & r, uint8_t & g, uint8_t & b )
    {
        const uint8_t * pal = fheroes2::getGamePalette() + static_cast<ptrdiff_t>( colorId ) * 3;
        r = static_cast<uint8_t>( pal[0] << 2 );
        g = static_cast<uint8_t>( pal[1] << 2 );
        b = static_cast<uint8_t>( pal[2] << 2 );
    }

    // Scale a game-pixel point to physical-pixel space.
    fheroes2::Point scalePoint( const fheroes2::Point & pt, const float scale )
    {
        return { static_cast<int32_t>( static_cast<float>( pt.x ) * scale ), static_cast<int32_t>( static_cast<float>( pt.y ) * scale ) };
    }

    void RedrawLightningRGBA( const std::vector<std::pair<LightningPoint, LightningPoint>> & lightning, const uint8_t color, fheroes2::Image & surface,
                              const float scale, const fheroes2::Point & physOrigin = fheroes2::Point(), const fheroes2::Rect & roi = fheroes2::Rect() )
    {
        ...
        for ( const auto & [firstPoint, secondPoint] : lightning ) {
            ...
            fheroes2::DrawLineRGBA( surface, first, second, r, g, b );

            for ( int32_t thickness = 1; thickness < secondPoint.thickness; ++thickness ) {
                const bool isUpper = ( ( thickness % 2 ) == 1 );
                const int32_t offset = isUpper ? static_cast<int32_t>( static_cast<float>( ( thickness + 1 ) / 2 ) * thicknessScale )
                                               : -static_cast<int32_t>( static_cast<float>( ( thickness + 1 ) / 2 ) * thicknessScale );

                if ( isHorizontal ) {
                    fheroes2::DrawLineRGBA( surface, { first.x, first.y + offset }, { second.x, second.y + offset }, r, g, b );
                }
                else {
                    fheroes2::DrawLineRGBA( surface, { first.x + offset, first.y }, { second.x + offset, second.y }, r, g, b );
                }
            }
            ...
            for ( int32_t thickness = secondPoint.thickness; thickness < firstPoint.thickness; ++thickness ) {
                ...
                if ( isHorizontal ) {
                    fheroes2::DrawLineRGBA( surface, { first.x, first.y + offset }, second, r, g, b );
                }
                else {
                    fheroes2::DrawLineRGBA( surface, { first.x + offset, first.y }, second, r, g, b );
                }
            }
        }
        (void)roi; // ROI clipping not implemented for RGBA path yet.
    }
...
bool Battle::OpponentSprite::isCaptain() const
{
    return _heroBase != nullptr && _heroBase->isCaptain();
}
...
void Battle::TurnOrder::addCustomMonsterOverlays() const
{
    for ( const UnitPos & entry : _rects ) {
        ...
        const fheroes2::Image * portrait = fheroes2::AGG::GetRGBACustomPortrait( unit->GetID() );
...
void Battle::TurnOrder::redraw( const Unit * current, const uint8_t currentUnitColor, const Unit * underCursor, const fheroes2::Rect & dialogRoi )
{
    if ( _restorer ) {
        _restorer->restore();
    }
    ...
    if ( _rects.size() != static_cast<size_t>( unitsToDraw ) ) {
        ...
        int32_t rectX = _renderingRoi.x;
        for ( int32_t index = 0; index < unitsToDraw; ++index ) {
            _rects.emplace_back( nullptr, fheroes2::Rect( rectX, _renderingRoi.y, turnOrderMonsterIconSize, turnOrderMonsterIconSize ) );
            rectX += turnOrderMonsterIconSize;
        }
    ...
    for ( const Unit * unit : *unitsOrder ) {
        ...
        _redrawUnit( _rects[unitRectIndex].second, *unit, unit->GetColor() == _opponentColor, unitColor, display );
    ...
    : arena( battleArena )
{
    ...
    // Cache battle area dimensions in GAME pixels (pure-RGBA Display is at game resolution).
    _battleAreaWidthPx = area.width;
    _battleAreaHeightPx = battlefieldHeight;
    (void)display;
...
void Battle::Interface::redrawPreRender()
{
    ...
    if ( IS_DEVEL() ) {
        ...
        auto & debugDisplay = fheroes2::Display::instance();
        for ( const Cell & cell : *board ) {
            const fheroes2::Text text( std::to_string( cell.GetIndex() ), fheroes2::FontType::smallWhite() );
            text.draw( _interfacePosition.x + cell.GetPos().x + 20, _interfacePosition.y + cell.GetPos().y + 24, debugDisplay );
        }
    }
#endif

    // Layer hi-res portraits for custom monsters (Thor, Succubus, ...) in the turn order.
    // With pure-RGBA Display, these write directly to Display in execution order — Z-order
    ...
    for ( int32_t cellRowId = 0; cellRowId < Board::heightInCells; ++cellRowId ) {
        ...
        if ( castle != nullptr ) {
            ...
            for ( const Battle::UnitSpellEffectInfo * overlaySprite : troopOverlaySpriteBeforeWall ) {
                ...
                _blitOnSurface( spellSprite, overlaySprite->position.x, overlaySprite->position.y, overlaySprite->isReflectedImage );
            }
...
void Battle::Interface::RedrawOpponents()
{
    if ( _attackingOpponent ) {
        const fheroes2::Sprite & hero = fheroes2::AGG::GetICN( _attackingOpponent->getHeroIcnId(), _attackingOpponent->getFrame() );
        fheroes2::Point offset( _attackingOpponent->Offset() );
        if ( _attackingOpponent->isCaptain() ) {
            offset.x -= OpponentSprite::CAPTAIN_X_OFFSET;
            offset.y += OpponentSprite::CAPTAIN_Y_OFFSET;
        }
        _blitOnSurface( hero, offset.x + OpponentSprite::LEFT_HERO_X_OFFSET + hero.x(), offset.y + OpponentSprite::LEFT_HERO_Y_OFFSET + hero.y() );
    }
    if ( _defendingOpponent ) {
        const fheroes2::Sprite & hero = fheroes2::AGG::GetICN( _defendingOpponent->getHeroIcnId(), _defendingOpponent->getFrame() );
        fheroes2::Point offset( _defendingOpponent->Offset() );
        if ( _defendingOpponent->isCaptain() ) {
            offset.x += OpponentSprite::CAPTAIN_X_OFFSET;
            offset.y += OpponentSprite::CAPTAIN_Y_OFFSET;
        }
        _blitOnSurface( hero, offset.x + fheroes2::Display::DEFAULT_WIDTH - OpponentSprite::RIGHT_HERO_X_OFFSET - ( hero.x() + hero.width() ),
                         offset.y + OpponentSprite::RIGHT_HERO_Y_OFFSET + hero.y(), true );
    }
...
void Battle::Interface::RedrawOpponentsFlags()
{
    ...
    if ( _attackingOpponent ) {
        ...
        _blitOnSurface( flag, _attackingOpponent->Offset().x + OpponentSprite::LEFT_HERO_X_OFFSET + flag.x(),
...
void Battle::Interface::RedrawTroopSprite( const Unit & unit )
{
    ...
    const std::vector<fheroes2::Image> * rgbaCustomFramesPtr = isCurrentMonsterAction ? nullptr : fheroes2::AGG::GetRGBACustomFrames( unit.GetID() );
    const bool isCustomRGBA = ( rgbaCustomFramesPtr != nullptr );

    fheroes2::Point drawnPosition;

    const fheroes2::Image * customRGBASrc = nullptr;
    ...
    if ( unit.Modes( SP_STONE | CAP_MIRRORIMAGE ) ) {
        ...
        if ( frame >= 0 && frame < static_cast<int32_t>( frames.size() ) && !frames[frame].empty() ) {
            ...
            // RGBA paint lands on top in execution order.
            fheroes2::Display & display = fheroes2::Display::instance();
            const int32_t dstX = drawnPosition.x + _interfacePosition.x;
            const int32_t dstY = drawnPosition.y + _interfacePosition.y;
            const int32_t dstW = monsterSprite.width();
            const int32_t dstH = monsterSprite.height();
            fheroes2::BlitRGBAScaled( frames[frame], display, dstX, dstY, dstW, dstH, unit.isReflect() );
        ...
        // Normal monsters: draw indexed sprite to both 8-bit and RGBA via wrapper.
        drawnPosition = _drawTroopSprite( unit, monsterSprite, false );
    }
    ...
    if ( needsContour ) {
        ...
        if ( customRGBASrc != nullptr && customRGBASrc->width() != monsterSprite.width() ) {
            const fheroes2::Image & rgbaSrc = *customRGBASrc;
            ...
            const float scaleY = static_cast<float>( rgbaSrc.height() ) / static_cast<float>( gameH );

            uint8_t * maskImg = mask.image();
            uint8_t * maskTr = mask.transform();
            const uint8_t * rgbaData = rgbaSrc.image();

            for ( int32_t y = 0; y < gameH; ++y ) {
                for ( int32_t x = 0; x < gameW; ++x ) {
                    const int32_t srcX = static_cast<int32_t>( static_cast<float>( x ) * scaleX );
                    const int32_t srcY = static_cast<int32_t>( static_cast<float>( y ) * scaleY );
                    const uint8_t alpha = rgbaData[( static_cast<ptrdiff_t>( srcY ) * rgbaSrc.width() + srcX ) * 4 + 3];
                    const ptrdiff_t idx = static_cast<ptrdiff_t>( y ) * gameW + x;
                    if ( alpha > 128 ) {
                        maskImg[idx] = 1;
                        maskTr[idx] = 0;
                    }
            ...
            const fheroes2::Sprite contour = fheroes2::CreateContour( mask, contourColor );
            _blitOnSurface( contour, drawnPosition.x, drawnPosition.y, unit.isReflect() );
        }
        else {
            // For palette sprites, use the palette sprite for contour.
...
fheroes2::Point Battle::Interface::_drawTroopSprite( const Unit & unit, const fheroes2::Sprite & troopSprite, const bool skipBlit )
{
    ...
    _alphaBlitOnSurface( troopSprite, offset.x, offset.y, unit.GetCustomAlpha(), unit.isReflect() );
...
{
    fheroes2::Display & display = fheroes2::Display::instance();
    fheroes2::Blit( in, display, outX + _interfacePosition.x, outY + _interfacePosition.y, flip );
}
...
{
    fheroes2::Display & display = fheroes2::Display::instance();
    fheroes2::AlphaBlit( in, 0, 0, display, outX + _interfacePosition.x, outY + _interfacePosition.y, in.width(), in.height(), alpha, flip );
}
...
{
    fheroes2::Display & display = fheroes2::Display::instance();
    fheroes2::AlphaBlit( in, inX, inY, display, outX + _interfacePosition.x, outY + _interfacePosition.y, w, h, alpha, flip );
}
...
{
    fheroes2::Display & display = fheroes2::Display::instance();
    // Use Blit (which auto-handles indexed->RGBA) and crops to (w,h) at game-pixel coords.
    fheroes2::Blit( in, inX, inY, display, outX + _interfacePosition.x, outY + _interfacePosition.y, w, h );
}

void Battle::Interface::_copyFullSurface()
{
    // Fast restore of the pre-rendered battlefield background to Display at the battle origin.
    if ( _battleGroundRGBA.empty() ) {
        return;
    }
    fheroes2::Display & display = fheroes2::Display::instance();
    fheroes2::Copy( _battleGroundRGBA, 0, 0, display, _interfacePosition.x, _interfacePosition.y, _battleGroundRGBA.width(), _battleGroundRGBA.height() );
}


const fheroes2::Image & Battle::Interface::_getOrConvertToRGBA( const fheroes2::Sprite & sprite, const int icnId, const int frameIndex )
{
    ...
    entry = fheroes2::Image( sprite.width(), sprite.height(), fheroes2::ImageFormat::RGBA_32BIT );
    entry.reset();
    // Indexed->RGBA conversion via Copy hits CopyIndexedToRGBAOutput.
    fheroes2::Copy( sprite, 0, 0, entry, 0, 0, sprite.width(), sprite.height() );
    return entry;
}
                                                      ...
                                                      const fheroes2::Point & movementDelta, const CellDirection movementDirection )
{
    ...
    // Top
    _alphaBlitOnSurface( sprite, 0, 0, spriteRect.x, spriteRect.y, spriteRect.width, intersection.y - spriteRect.y, alpha, isReflect );

    // Left
    _alphaBlitOnSurface( sprite, 0, intersection.y - spriteRect.y, spriteRect.x, intersection.y, intersection.x - spriteRect.x, intersection.height, alpha, isReflect );

    // Right
    _alphaBlitOnSurface( sprite, ( intersection.x + intersection.width ) - spriteRect.x, intersection.y - spriteRect.y, intersection.x + intersection.width,
...
void Battle::Interface::RedrawTroopCount( const Unit & unit )
{
    ...
    // Draw text onto a copy of the bar, then blit the combined image.
    const fheroes2::Text text( fheroes2::abbreviateNumber( static_cast<int32_t>( unit.GetCount() ) ), fheroes2::FontType::smallWhite() );
    fheroes2::Image barWithText( bar.width(), bar.height() );
    barWithText._disableTransformLayer();
    fheroes2::Copy( bar, 0, 0, barWithText, 0, 0, bar.width(), bar.height() );
    text.draw( ( bar.width() - text.width() ) / 2, 2, barWithText );
    _copyOnSurface( barWithText, 0, 0, sx, sy, barWithText.width(), barWithText.height() );
}
...
void Battle::Interface::RedrawCover()
{
    ...
    if ( bridge && ( bridge->isDown() || _bridgeAnimation.animationIsRequired ) ) {
        ...
        _blitOnSurface( bridgeImage, bridgeImage.x(), bridgeImage.y() );
    }
    ...
    if ( cell && _currentUnit && Settings::Get().BattleShowMouseShadow() ) {
        ...
        for ( const Cell * highlightedCell : highlightedCells ) {
            ...
            if ( isApplicable ) {
                _blitOnSurface( _hexagonCursorShadow, highlightedCell->GetPos().x, highlightedCell->GetPos().y );
            }
        }
    }
}
...
{
    // Build the battlefield background as a game-resolution RGBA image directly into _battleGroundRGBA.
    const int32_t bgW = _surfaceInnerArea.width;
    const int32_t bgH = ( _battleAreaHeightPx > 0 ) ? _battleAreaHeightPx : _surfaceInnerArea.height;
    ...
    if ( _battleGroundIcn != ICN::UNKNOWN ) {
        ...
        fheroes2::Copy( cbkg, 0, 0, bgRGBA, 0, 0, std::min( cbkg.width(), bgW ), std::min( cbkg.height(), bgH ) );
    }
    ...
    if ( _borderObjectsIcn != ICN::UNKNOWN ) {
        ...
        fheroes2::Blit( frng, bgRGBA, frng.x(), frng.y() );
    }
    ...
    if ( arena.GetICNCovr() != ICN::UNKNOWN ) {
        ...
        fheroes2::Blit( cover, bgRGBA, cover.x(), cover.y() );
    }
    ...
    if ( castle != nullptr ) {
        ...
        fheroes2::Blit( castleBackground, bgRGBA, castleBackground.x(), castleBackground.y() );

        // Moat.
        if ( castle->isBuild( BUILD_MOAT ) ) {
            const fheroes2::Sprite & sprite = fheroes2::AGG::GetICN( ICN::MOATWHOL, 0 );
            fheroes2::Blit( sprite, bgRGBA, sprite.x(), sprite.y() );
        }
    }
    ...
    if ( Settings::Get().BattleShowGrid() ) {
        ...
        for ( const Cell & cell : board ) {
            fheroes2::Blit( _hexagonGrid, bgRGBA, cell.GetPos().x, cell.GetPos().y );
        }
    }

    // Ground obstacles.
    _battleGroundBuildTarget = &bgRGBA;
    for ( int32_t cellId = 0; cellId < Board::sizeInCells; ++cellId ) {
        _redrawGroundObjects( cellId );
    }
    _battleGroundBuildTarget = nullptr;
    ...
    // Pure-RGBA Display: _battleGroundRGBA is the same game-resolution buffer.
    _battleGroundRGBA = std::move( bgRGBA );
}
...
void Battle::Interface::_redrawCoverStatic()
{
    ...
    if ( _highlightUnitMovementArea != nullptr ) {
        ...
        for ( const Cell & cell : board ) {
            ...
            if ( pos.GetHead() != nullptr ) {
                ...
                _blitOnSurface( _hexagonHighlightShadow, cell.GetPos().x, cell.GetPos().y );
                _blitOnSurface( _hexagonHighlightShadow, cell.GetPos().x, cell.GetPos().y );
    ...
    for ( const Cell & cell : board ) {
        ...
        if ( pos.GetHead() != nullptr ) {
            ...
            _blitOnSurface( shadowImage, cell.GetPos().x, cell.GetPos().y );
        }
    }
}
...
void Battle::Interface::RedrawCastle( const Castle & castle, const int32_t cellId )
{
    ...
    if ( Arena::CATAPULT_POS == cellId ) {
        ...
        _blitOnSurface( sprite, 22 + sprite.x(), 390 + sprite.y() );
    }
        ...
        if ( bridge != nullptr && !bridge->isDestroyed() ) {
            ...
            _blitOnSurface( sprite, sprite.x(), sprite.y() );
        }
    }
        ...
        _blitOnSurface( towerSprite, 443 + towerSprite.x(), 153 + towerSprite.y() );
    }
        ...
        const fheroes2::Sprite & towerSprite = fheroes2::AGG::GetICN( castleIcnId, index );
        _blitOnSurface( towerSprite, 443 + towerSprite.x(), 405 + towerSprite.y() );
    }
    else if ( Arena::CASTLE_TOP_GATE_TOWER_POS == cellId ) {
        const int index = ( Board::GetCell( cellId )->GetObject() == 1 ) ? 19 : 17;
        const fheroes2::Sprite & towerSprite = fheroes2::AGG::GetICN( castleIcnId, index );
        _blitOnSurface( towerSprite, 399 + towerSprite.x(), 237 + towerSprite.y() );
    }
    else if ( Arena::CASTLE_BOTTOM_GATE_TOWER_POS == cellId ) {
        const int index = ( Board::GetCell( cellId )->GetObject() == 1 ) ? 19 : 17;
        const fheroes2::Sprite & towerSprite = fheroes2::AGG::GetICN( castleIcnId, index );
        _blitOnSurface( towerSprite, 399 + towerSprite.x(), 321 + towerSprite.y() );
    }
}
...
void Battle::Interface::_redrawGroundObjects( const int32_t cellId )
{
    ...
    fheroes2::Image & groundTarget = _battleGroundBuildTarget ? *_battleGroundBuildTarget : fheroes2::Display::instance();
    fheroes2::Blit( objectSprite, groundTarget, pt.x + pt.width / 2 + objectSprite.x(), pt.y + pt.height + objectSprite.y() + cellYOffset );
}
...
void Battle::Interface::_redrawHighObjects( const int32_t cellId )
{
    ...
    const fheroes2::Sprite & objectSprite = fheroes2::AGG::GetICN( objectIcnId, 0 );
    const fheroes2::Rect & pt = cell->GetPos();
    _blitOnSurface( objectSprite, pt.x + pt.width / 2 + objectSprite.x(), pt.y + pt.height + objectSprite.y() + cellYOffset );
}
        ...
        if ( arena.GetCurrentColor() == arena.getAttackingArmyColor() ) {
            ...
            if ( le.MouseClickLeft( attackingOpponentArea ) ) {
                int heroResult;
                {
                    ...
                    heroResult = arena.DialogBattleHero( *_attackingOpponent->GetHero(), true, status );
                }
                ProcessingHeroDialogResult( heroResult, actions );
                humanturn_redraw = true;
            }
        }
        else {
        ...
        if ( arena.GetCurrentColor() == arena.getDefendingForce().GetColor() ) {
            ...
            if ( le.MouseClickLeft( defendingOpponentArea ) ) {
                int heroResult;
                {
                    ...
                    heroResult = arena.DialogBattleHero( *_defendingOpponent->GetHero(), true, status );
                }
                ProcessingHeroDialogResult( heroResult, actions );
                humanturn_redraw = true;
            }
    ...
    // mirrored into _screenRGBA by the WriteHook; the loop below repeatedly dims that area.
    {
        ...
        while ( le.HandleEvents( Game::isCustomDelayNeeded( stepDelay ) ) ) {
            if ( Game::validateCustomAnimationDelay( stepDelay ) ) {
                ...
                // Darken the battle area on Display directly (game-pixel coords).
                {
                    fheroes2::Display & dsp = fheroes2::Display::instance();
                    fheroes2::DimRGBA( dsp, _interfacePosition.x, _interfacePosition.y, _battleAreaWidthPx, _battleAreaHeightPx, dimFactor );
                }
...
void Battle::Interface::RedrawMissileAnimation( const fheroes2::Point & startPos, const fheroes2::Point & endPos, const double angle, const uint32_t monsterID )
{
    ...
    while ( le.HandleEvents( false ) && pnt != points.end() ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_MISSILE_DELAY ) ) {
            RedrawPartialStart();
            if ( isMage ) {
                // RGBA missile beam lines on Display directly (game-pixel coords).
                fheroes2::Display & dsp = fheroes2::Display::instance();
                if ( !dsp.empty() ) {
                    ...
                    const fheroes2::Point sp{ startPos.x + _interfacePosition.x, startPos.y + _interfacePosition.y };
                    const fheroes2::Point ep{ pnt->x + _interfacePosition.x, pnt->y + _interfacePosition.y };
                    fheroes2::DrawLineRGBA( dsp, { sp.x, sp.y - 2 }, { ep.x, ep.y - 2 }, r77, g77, b77 );
                    fheroes2::DrawLineRGBA( dsp, { sp.x, sp.y - 1 }, { ep.x, ep.y - 1 }, rB5, gB5, bB5 );
                    fheroes2::DrawLineRGBA( dsp, sp, ep, rBC, gBC, bBC );
                    fheroes2::DrawLineRGBA( dsp, { sp.x, sp.y + 1 }, { ep.x, ep.y + 1 }, rB5, gB5, bB5 );
                    fheroes2::DrawLineRGBA( dsp, { sp.x, sp.y + 2 }, { ep.x, ep.y + 2 }, r77, g77, b77 );
                }
            }
                ...
                _blitOnSurface( missile, pnt->x + missileOffset.x, pnt->y + missileOffset.y, reverse );
            }
            RedrawPartialFinish();
            ++pnt;
        }
...
void Battle::Interface::_redrawActionWincesKills( const TargetsInfo & targets, Unit * attacker /* = nullptr */, const Unit * defender /* = nullptr */ )
{
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_FRAME_DELAY } ) ) ) {
        ...
        if ( drawLichCloud && lichCloudFrame < lichCloudMaxFrame ) {
            ...
            _blitOnSurface( spellSprite, pos.x, pos.y, false );
            ++lichCloudFrame;
        }
    ...
    if ( isGoodLuck ) {
        ...
        while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_MISSILE_DELAY } ) )
            ...
            if ( Game::validateAnimationDelay( Game::BATTLE_MISSILE_DELAY ) ) {
                ...
                if ( isVerticalRainbow ) {
                    _copyOnSurface( luckSprite, 0, 0, drawOffset, 0, luckSprite.width(), drawWidth );
                }
                else {
                    if ( isRainbowFromRight ) {
                        _copyOnSurface( luckSprite, rainbowLength - drawWidth, 0, battleArea.width - drawWidth, drawOffset, drawWidth, luckSprite.height() );
                    }
                    else {
                        _copyOnSurface( luckSprite, 0, 0, 0, drawOffset, drawWidth, luckSprite.height() );
                    }
                }
        ...
        while ( le.HandleEvents( Game::isCustomDelayNeeded( animationDelay ) ) && ( frameId < frameLimit || ( soundOn && Mixer::isPlaying( -1 ) ) ) ) {
            ...
            if ( Game::validateCustomAnimationDelay( animationDelay ) ) {
                ...
                if ( frameId < frameLimit ) {
                    ...
                    _blitOnSurface( luckSprite, offsetX + luckSprite.x(), offsetY + luckSprite.y() );
...
void Battle::Interface::RedrawActionCatapultPart1( const CastleDefenseStructure catapultTarget, const bool isHit )
{
    ...
    while ( le.HandleEvents( false ) && pnt != points.end() ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_CATAPULT_BOULDER_DELAY ) ) {
            ...
            RedrawPartialStart();
            _blitOnSurface( fheroes2::AGG::GetICN( ICN::BOULDER, boulderFrameId ), pnt->x, pnt->y );
            RedrawPartialFinish();
            ++pnt;
            ++boulderFrameId;
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_CATAPULT_CLOUD_DELAY } ) ) && frame < maxFrame ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_CATAPULT_CLOUD_DELAY ) ) {
            ...
            if ( isBridgeDestroyed && frame >= bridgeDestroySmokeDelay ) {
                ...
                _blitOnSurface( sprite, pt1.x + sprite.x(), pt1.y + sprite.y() );
            }
            const fheroes2::Sprite & sprite = fheroes2::AGG::GetICN( icn, frame );
            _blitOnSurface( sprite, pt2.x + sprite.x(), pt2.y + sprite.y() );
            RedrawPartialFinish();
    ...
    for ( size_t i = 1; i < points.size(); ++i ) {
        ...
        while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) )
            ...
            if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
                ...
                // Draw lightning and dim the battle area on Display directly (game-pixel coords).
                {
                    fheroes2::Display & dsp = fheroes2::Display::instance();
                    if ( !dsp.empty() ) {
                        RedrawLightningRGBA( lightningBolt, fheroes2::GetColorId( 0xff, 0xff, 0 ), dsp, 1.0f, { _interfacePosition.x, _interfacePosition.y } );
                        fheroes2::DimRGBA( dsp, _interfacePosition.x, _interfacePosition.y, _battleAreaWidthPx, _battleAreaHeightPx, 0.65f );
                    }
                }
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && frame < fheroes2::AGG::GetICNCount( ICN::SPARKS ) ) {
        ...
        if ( ( frame == 0 ) || Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            ...
            for ( size_t i = 1; i < points.size(); ++i ) {
                ...
                _blitOnSurface( sprite, pt.x, pt.y );
            }
            RedrawPartialFinish();
...
void Battle::Interface::_redrawRaySpell( const Unit & target, const int spellICN, const int spellSound, const int32_t size )
{
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && i < path.size() ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            ...
            _blitOnSurface( sprite, path[i].x - sprite.width() / 2, path[i].y - sprite.height() / 2 );
            RedrawPartialFinish();
            ++i;
        }
    }
...
void Battle::Interface::_redrawActionDeathWaveSpell( const int32_t strength )
{
    ...
    // Set all non-dead troops animation to static and redraw the battlefield.
    ...
    fheroes2::Display & display = fheroes2::Display::instance();
    const int32_t physOX = _interfacePosition.x;
    const int32_t physOY = _interfacePosition.y;
    const int32_t rgbaAreaW = area.width;
    const int32_t rgbaAreaH = area.height;

    fheroes2::Image battleFieldCopyRGBA( rgbaAreaW, rgbaAreaH, fheroes2::ImageFormat::RGBA_32BIT );
    fheroes2::CopyRGBA( display, physOX, physOY, battleFieldCopyRGBA, 0, 0, rgbaAreaW, rgbaAreaH );

    // The death wave horizontal length in game-resolution pixels.
    ...
    for ( int32_t posX = 0; posX < waveLength; ++posX ) {
        deathWaveCurve.push_back( static_cast<int32_t>( std::round( strength * ( cosf( posX / waveLimit ) / 2.0f - 0.5f ) ) ) - 1 );
    }
    ...
    fheroes2::Image spellEffectRGBA( waveLength, rgbaAreaH, fheroes2::ImageFormat::RGBA_32BIT );

    AudioManager::PlaySound( M82::MNRDEATH );

    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && position < rgbaAreaW + waveLength ) {
        CheckGlobalEvents( le );

        if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            const int32_t wavePositionX = ( rgbaAreaX - waveLength + position < 0 ) ? 0 : ( rgbaAreaX - waveLength + position );
            const int32_t waveWidth = position > waveLength ? ( position > rgbaAreaW ? ( waveLength - position + rgbaAreaW ) : waveLength ) : position;
            ...
            // Restore the previous wave position on Display.
            fheroes2::CopyRGBA( battleFieldCopyRGBA, restorePositionX, 0, display, physOX + restorePositionX, physOY, restoreWidth, rgbaAreaH );

            // Apply the Death Wave effect and write into Display at the battle origin + offset.
            fheroes2::CreateDeathWaveEffectRGBA( spellEffectRGBA, battleFieldCopyRGBA, position, deathWaveCurve );
            fheroes2::CopyRGBA( spellEffectRGBA, 0, 0, display, physOX + restorePositionX + restoreWidth, physOY, waveWidth, rgbaAreaH );

            // Render the changed area.
            const fheroes2::Rect renderArea( _interfacePosition.x + restorePositionX, _interfacePosition.y, waveWidth + restoreWidth + 1, area.height );
...
void Battle::Interface::_redrawActionColdRingSpell( const int32_t dst, const TargetsInfo & targets )
{
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && frame < fheroes2::AGG::GetICNCount( icn ) ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            ...
            _blitOnSurface( sprite1, center.x + center.width / 2 + sprite1.x(), center.y + center.height / 2 + sprite1.y() );
            const fheroes2::Sprite & sprite2 = fheroes2::AGG::GetICN( icn, frame );
            _blitOnSurface( sprite2, center.x + center.width / 2 - sprite2.width() - sprite2.x(), center.y + center.height / 2 + sprite2.y(), true );
            RedrawPartialFinish();
...
void Battle::Interface::_redrawActionHolyShoutSpell( const uint8_t strength )
{
    ...
    // Capture the RGBA battlefield from Display directly (game-pixel coords).
    fheroes2::Display & display = fheroes2::Display::instance();
    const int32_t physOX = _interfacePosition.x;
    const int32_t physOY = _interfacePosition.y;
    ...
    fheroes2::Image battleFieldCopyRGBA( rgbaW, rgbaH, fheroes2::ImageFormat::RGBA_32BIT );
    fheroes2::CopyRGBA( display, physOX, physOY, battleFieldCopyRGBA, 0, 0, rgbaW, rgbaH );
    ...
    // Game-resolution blur radius.
    const int32_t scaledBlurRadius = 4;
    ...
    while ( spellEffect.size() < spellEffectLastFrame ) {
        fheroes2::Image frameCopy( rgbaW, rgbaH, fheroes2::ImageFormat::RGBA_32BIT );
        fheroes2::CopyRGBA( battleFieldCopyRGBA, 0, 0, frameCopy, 0, 0, rgbaW, rgbaH );
        spellEffect.push_back( std::move( frameCopy ) );
    }

    // The last frame is the full power of spell effect.
    spellEffect.emplace_back( fheroes2::CreateHolyShoutEffectRGBA( battleFieldCopyRGBA, scaledBlurRadius, strength ) );
        ...
        if ( Game::validateCustomAnimationDelay( spellcastDelay ) ) {
            if ( frame != halfMaxFrame ) {
                if ( frame < spellEffectLastFrame ) {
                    // Blend the full-effect frame into the current frame at increasing alpha.
                    fheroes2::BlitRGBAAlpha( spellEffect[spellEffectLastFrame], spellEffect[frame], 0, 0, alpha );
                    alpha += alphaStep;
                }

                const uint32_t spellEffectFrame = ( frame < halfMaxFrame ) ? frame : ( maxFrame - frame - 1 );
                // Write the effect frame to Display at the battle origin (game-pixel coords).
                fheroes2::CopyRGBA( spellEffect[spellEffectFrame], 0, 0, display, physOX, physOY, rgbaW, rgbaH );
            ...
            if ( icnCount > 0 ) {
                for ( int x = 0; x * spriteSize < _surfaceInnerArea.width; ++x ) {
                    ...
                    for ( int y = 0; y * spriteSize < _surfaceInnerArea.height; ++y ) {
                        ...
                        _blitOnSurface( sprite, offsetX + sprite.x(), y * spriteSize + sprite.y() );
                    }
                }
            }
...
void Battle::Interface::_redrawActionArmageddonSpell()
{
    ...
    fheroes2::Image spriteWhiteningRGBA( rgbaW, rgbaH, fheroes2::ImageFormat::RGBA_32BIT );
    fheroes2::CopyRGBA( display, physOX, physOY, spriteWhiteningRGBA, 0, 0, rgbaW, rgbaH );

    // Create the reddish version by dimming green and blue channels.
    fheroes2::Image spriteReddishRGBA( rgbaW, rgbaH, fheroes2::ImageFormat::RGBA_32BIT );
    fheroes2::CopyRGBA( spriteWhiteningRGBA, 0, 0, spriteReddishRGBA, 0, 0, rgbaW, rgbaH );
    fheroes2::DimRGBA( spriteReddishRGBA, 0, 0, rgbaW, rgbaH, 0.7f );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && alpha < 100 ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            ...
            fheroes2::CopyRGBA( spriteWhiteningRGBA, 0, 0, display, physOX, physOY, rgbaW, rgbaH );
            RedrawPartialFinish();
    ...
    fheroes2::CopyRGBA( spriteReddishRGBA, 0, 0, display, physOX, physOY, rgbaW, rgbaH );

    const int32_t rgbaShakeX = 14;
    const int32_t rgbaShakeY = 11;
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && Mixer::isPlaying( -1 ) ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            ...
            fheroes2::CopyRGBA( spriteReddishRGBA, std::max<int32_t>( 0, -offsetX ), std::max<int32_t>( 0, -offsetY ), display,
...
void Battle::Interface::redrawActionEarthquakeSpellPart1( const HeroBase & caster, const std::vector<CastleDefenseStructure> & targets )
{
    ...
    fheroes2::Image battlefieldRGBA( rgbaW, rgbaH, fheroes2::ImageFormat::RGBA_32BIT );
    fheroes2::CopyRGBA( dsp, physOX, physOY, battlefieldRGBA, 0, 0, rgbaW, rgbaH );
    ...
    // Draw earthquake animation by shaking the Display battle region.
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && frame < 18 ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            ...
            fheroes2::CopyRGBA( battlefieldRGBA, std::max<int32_t>( 0, -offsetX ), std::max<int32_t>( 0, -offsetY ), dsp,
                                physOX + std::max<int32_t>( 0, offsetX ), physOY + std::max<int32_t>( 0, offsetY ), rgbaW - std::abs( offsetX ),
                                rgbaH - std::abs( offsetY ) );
        ...
        if ( isNeedSpellAnimationUpdate || Game::validateAnimationDelay( Game::BATTLE_OPPONENTS_DELAY ) ) {
            ...
            for ( const CastleDefenseStructure target : targets ) {
                ...
                if ( target == CastleDefenseStructure::BRIDGE && frame >= bridgeDestroySmokeDelay ) {
                    ...
                    const fheroes2::Sprite & sprite = fheroes2::AGG::GetICN( icn, frame - bridgeDestroySmokeDelay );
                    _blitOnSurface( sprite, pt2.x + bridgeDestroySmokeOffset.x + sprite.x(), pt2.y + bridgeDestroySmokeOffset.y + sprite.y() );
                }

                const fheroes2::Sprite & spriteCloud = fheroes2::AGG::GetICN( icn, frame );
                _blitOnSurface( spriteCloud, pt2.x + spriteCloud.x(), pt2.y + spriteCloud.y() );
            }
...
void Battle::Interface::RedrawTargetsWithFrameAnimation( const int32_t dst, const TargetsInfo & targets, const int icn, const int m82, int repeatCount /* = 0 */ )
{
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && frame < frameCount ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            ...
            const fheroes2::Sprite & sprite = fheroes2::AGG::GetICN( icn, frame );
            _blitOnSurface( sprite, center.x + center.width / 2 + sprite.x(), center.y + center.height / 2 + sprite.y() );
            RedrawPartialFinish();
...
void Battle::Interface::InterruptAutoCombatIfRequested( LocalEvent & le )
{
    ...
    int interrupt;
    {
        ...
        interrupt = fheroes2::showStandardTextMessage( {}, _( "Are you sure you want to interrupt the auto combat mode?" ), Dialog::YES | Dialog::NO );
    }
