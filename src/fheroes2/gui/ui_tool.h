...
namespace fheroes2
{
    ...
    void CreateDeathWaveEffectRGBA( Image & out, const Image & in, int32_t x, const std::vector<int32_t> & deathWaveCurve );

    Image CreateHolyShoutEffect( const Image & in, const int32_t blurRadius, const uint8_t darkredStrength );
    Image CreateHolyShoutEffectRGBA( const Image & in, int32_t blurRadius, uint8_t darkredStrength );

    // Lerp all RGB values toward white by the given factor (0.0 = no change, 1.0 = fully white). Preserves alpha. Image must be RGBA_32BIT.
    void WhitenRGBA( Image & image, float factor );
