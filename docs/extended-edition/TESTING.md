...
Golden images must be deterministic. Tests should fix dimensions, scale, timing, animation frame, random seed and color-space assumptions. A golden update should be a deliberate review action, not an automatic response to failure.
...
These tests should default to local execution only. They must not upload screenshots or captured data. If private CI is added later, it needs an explicit review of storage, logs, retention and artifact settings before original data is supplied.
