...
This directory is the entry point for rebuilding the extended edition on a clean upstream base. It records decisions that should survive across
development sessions and keeps the migration from depending on chat history.
...
Public CI must work from a clean checkout without Ubisoft assets. It must not download, commit, render into published screenshots, or upload original
game artwork as an artifact. See [TESTING.md](TESTING.md) for the synthetic-data strategy.
